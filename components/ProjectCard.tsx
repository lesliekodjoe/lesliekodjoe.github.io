import { projects_data } from "@/utils/data";
import Image from "next/image";
import React from "react";
import { GoArrowRight } from "react-icons/go";
import TransitionLink from "./TransitionLink";

const ProjectCard = () => {
  return (
    <>
      {projects_data.map((project) => (
        <div className="w-full pt-16 md:px-48">
          <TransitionLink
            key={project.num}
            href={`/projects/${project.num}`}
            className="w-full"
          >
            <div className="px-5 bg-[#2C2C2C] w-full h-full md:h-[625px] border border-[#393939] rounded-xl overflow-y-hidden group hover:bg-[#444444] ease-in-out duration-500 md:px-0">
              <div className="py-8 md:px-12 md:py-10 md:flex items-center">
                <div className="mb-4">
                  <h4 className="text-xl text-left mb-1 font-semibold md:text-2xl">
                    {project.projectInfo.projectName}
                  </h4>
                  <p className="text-[#adadad] text-sm md:text-base">
                    {project.projectInfo.projectRole} -{" "}
                    {project.projectInfo.tech}
                  </p>
                </div>
                <GoArrowRight className="text-2xl md:ml-16 md:text-3xl group-hover:translate-x-5 ease-in-out duration-500" />
              </div>
              <div className="w-full flex items-center justify-center">
                <div className="md:w-[810px] group-hover:-translate-y-4 ease-in-out duration-500">
                  <Image
                    src={project.media.imageSrc}
                    alt="projCard1"
                    width="0"
                    height="0"
                    sizes="100vw"
                    className="w-full h-auto"
                    priority
                  />
                </div>
              </div>
            </div>
          </TransitionLink>
        </div>
      ))}
    </>
  );
};

export default ProjectCard;
